...
class ChannelPublisher:
    ...
    async def publish(self, product, media_paths: list[str]):
        ...
        markup = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🛍 Купить', web_app=WebAppInfo(url=webapp_url))]])
        ...
        if usable:
            ...
            control = await self.bot.send_message(settings.shop_channel_id, '🛍 Заказать этот товар:', reply_markup=markup)
