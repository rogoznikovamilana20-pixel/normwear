        ...
        if msg.photo:
            try:
                ...
                if path:
                    ...
                    log.info("Скачано фото для сообщения %s", msg.id)
            except Exception as e:
                log.warning("Не удалось скачать фото для сообщения %s: %s", msg.id, e)

        await on_post(text, local_photos)
        log.info("Обработано сообщение %s из канала поставщика", msg.id)
    except Exception as e:
        log.error("Ошибка обработки сообщения %s: %s", msg.id, e)
